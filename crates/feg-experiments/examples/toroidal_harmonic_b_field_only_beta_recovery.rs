...
use feg_case_studies::toroidal_harmonic_b::{
    run_toroidal_harmonic_b_field_only_beta_recovery, ToroidalHarmonicBConfig,
};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let config = ToroidalHarmonicBConfig {
        output_dir: Some(PathBuf::from(
            "out/examples/toroidal_harmonic_b_field_only_beta_recovery",
        )),
    ...
    println!("Toroidal magnetic-field-only beta recovery");
    println!(
