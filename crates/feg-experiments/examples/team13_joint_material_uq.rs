use feg_case_studies::team13::{
    run_team13_joint_material_uq, Team13JointMaterialUqConfig, Team13MapParityPdeResidualKind,
    Team13MapParityPriorKind, Team13MaterialPriorCalibrationMode,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let result = run_team13_joint_material_uq(&config)?;
    println!("TEAM 13 joint material-law UQ");
    ...
    println!(
        "  posterior: converged={} precision_nnz={} factor_nnz={}",
        result.posterior_converged, result.posterior_precision_nnz, result.posterior_factor_nnz
    );
    for report in &result.material_posterior {
        println!(
            "  {} @ {:.3} T: mean={:.6e} std={:.6e}",
            report.name, report.anchor_b_tesla, report.posterior_mean, report.posterior_std
        );
    }
...
fn parse_args() -> Result<Team13JointMaterialUqConfig, Box<dyn Error>> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--no-steel-observations" => config.operator.include_steel_observations = false,
...
fn print_help() {
    println!(
        "Usage: cargo run --release -p feg-case-studies --example team13_joint_material_uq -- [options]\n\
         Options:\n\
           --mesh <path>\n\
