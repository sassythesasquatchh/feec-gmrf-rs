...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output_dir = manifest_dir.join("../../out/planar_holes_incompressible_flow");
    let config = PlanarHolesFlowConfig {
        output_dir: output_dir.clone(),
        mesh_path: output_dir.join("planar_holes_incompressible.msh"),
        geo_path: output_dir.join("planar_holes_incompressible.geo"),
    ...
    println!("Planar holes incompressible Hodge-GMRF flow experiment");
    ...
    for scenario in &result.scenarios {
        for metric in &scenario.metrics {
            ...
            println!(
                "scenario={} model={} rel_l2={:.4e} nlpd={:.4e} loop_nlpd={:.4e} harmonic_period_nlpd={:.4e} delta_metric={:.4e} delta_kind={} rel_total_loop_error={:.4e} rel_harmonic_period_error={:.4e} rel_coexact_annular_error={:.4e} rel_total_annular_error={:.4e} field_cov95={:.3} field_mw_cov95={:.3} heldout_field_cov95={:.3} field_mean_abs_z={:.3e} field_p95_abs_z={:.3e}",
