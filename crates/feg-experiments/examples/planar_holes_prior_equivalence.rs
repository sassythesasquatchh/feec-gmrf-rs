use feg_case_studies::planar_holes_hodge_flow::{
    run_planar_holes_prior_equivalence, write_planar_holes_prior_equivalence,
    PlanarHolesFlowConfig, PlanarHolesPriorEquivalenceConfig,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output_dir = manifest_dir.join("../../out/planar_holes_prior_equivalence");
    fs::create_dir_all(&output_dir)?;

    let mesh_size = env::var("PLANAR_HOLES_PRIOR_EQUIV_MESH_SIZE")
        .ok()
        .and_then(|value| value.parse::<f64>().ok())
        .unwrap_or(0.14);
    let mode_counts = env::var("PLANAR_HOLES_PRIOR_EQUIV_MODES")
        .ok()
        .map(|value| parse_mode_counts(&value))
        .unwrap_or_else(|| vec![32, 64, 128]);
    let include_max_available = env::var("PLANAR_HOLES_PRIOR_EQUIV_MAX")
        .ok()
        .map(|value| value == "1" || value.eq_ignore_ascii_case("true"))
        .unwrap_or(true);
    let include_exact_lower_mass = env::var("PLANAR_HOLES_PRIOR_EQUIV_EXACT_LOWER")
        .ok()
        .map(|value| value == "1" || value.eq_ignore_ascii_case("true"))
        .unwrap_or(true);
    let dense_eigen_dimension_cap = env::var("PLANAR_HOLES_PRIOR_EQUIV_EIGEN_CAP")
        .ok()
    ...
    let base = PlanarHolesFlowConfig {
        ...
        mesh_path: output_dir.join("planar_holes_prior_equivalence.msh"),
        geo_path: output_dir.join("planar_holes_prior_equivalence.geo"),
        force_mesh: true,
        mesh_size,
    ...
    println!(
        "include_exact_lower_mass={}",
        config.include_exact_lower_mass
    );

    let rows = run_planar_holes_prior_equivalence(&config)?;
    write_planar_holes_prior_equivalence(&rows, output_dir.join("prior_equivalence.csv"))?;
