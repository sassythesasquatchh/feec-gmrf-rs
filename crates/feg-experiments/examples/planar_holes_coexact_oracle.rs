...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output_dir = manifest_dir.join("../../out/planar_holes_coexact_oracle");
    fs::create_dir_all(&output_dir)?;
    let mesh_path = output_dir.join("planar_holes_coexact_oracle.msh");
    let geo_path = output_dir.join("planar_holes_coexact_oracle.geo");
    let csv_path = output_dir.join("coexact_oracle_summary.csv");
    let cases = [
        OracleCase {
            name: "matched_sparse_noisy",
            truth_source: PlanarHolesCoexactTruthSource::SparseAnchor,
        ...
        OracleCase {
            name: "exact_mass_dense_noiseless",
            truth_source: PlanarHolesCoexactTruthSource::ExactMassCoexact,
            truth_scaling: PlanarHolesTruthScaling::RawPriorSamples,
            observation_design: PlanarHolesLocalObservationDesign::AllEdges,
            sample_observation_noise: false,
            local_noise_variance: NEAR_NOISELESS_VARIANCE,
    ...
    writeln!(
        writer,
        "case,truth_source,truth_scaling,observation_design,sample_observation_noise,scenario,model,observation_count,l2_error,heldout_local_nlpd,codifferential_leakage,all_edge_coverage_95,coexact_truth_norm,coexact_posterior_norm,coexact_relative_branch_error,coexact_mass_correlation"
    )?;

    println!("Planar holes pure-coexact oracle experiments");
    ...
    for (index, case) in cases.iter().enumerate() {
        ...
        let config = PlanarHolesFlowConfig {
            ...
            coexact_truth_source: case.truth_source,
        ...
        println!(
            "case={} topology: vertices={} edges={} faces={} b1={} truth_norm={:.4e} truth_coexact_norm={:.4e}",
            case.name,
            ...
            mass_norm_from_result(&result.truth, &result),
            mass_norm_from_result(&result.truth_coexact, &result),
        );
        for scenario in &result.scenarios {
            for metric in &scenario.metrics {
                ...
                if matches!(
                    ...
                    println!(
                        "case={} scenario={} model={} obs={} E1={:.4e} leakage={:.4e} field_cov={:.3} coexact_norm={:.4e} coexact_rel_err={:.4e} coexact_corr={:.4e}",
                        case.name,
                        metric.scenario.as_str(),
                        metric.model.as_str(),
                        metric.observation_count,
