use feg_case_studies::team13::{
    run_team13_identifiable_material_uq, Team13IdentifiableMaterialUqConfig,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let result = run_team13_identifiable_material_uq(&config)?;
    println!("TEAM 13 identifiable material UQ");
...
fn parse_args() -> Result<Team13IdentifiableMaterialUqConfig, Box<dyn Error>> {
    let mut config = Team13IdentifiableMaterialUqConfig::default();
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--max-eta-step-norm" => {
                config.max_eta_step_norm = next_arg(&mut args, "--max-eta-step-norm")?.parse()?
            }
            "--prior-kind" => {
...
fn print_help() {
    println!(
        "Usage: cargo run --release -p feg-case-studies --example team13_identifiable_material_uq -- [options]\n\
         Options:\n\
           --mesh <path>\n\
