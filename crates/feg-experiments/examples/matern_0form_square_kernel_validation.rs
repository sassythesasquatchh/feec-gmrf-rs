use feg_case_studies::square_zero_form_kernel_validation::{
    compute_square_zero_form_kernel_validation_report,
...
struct CliConfig {
    out_dir: PathBuf,
    experiment: SquareZeroFormKernelValidationConfig,
}

impl Default for CliConfig {
    fn default() -> Self {
        Self {
            out_dir: PathBuf::from("out/matern_0form_square_kernel_validation"),
            experiment: SquareZeroFormKernelValidationConfig::default(),
        }
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let report = compute_square_zero_form_kernel_validation_report(cli.experiment)
        .map_err(|err| format!("square 0-form kernel validation failed: {err}"))?;
    write_square_zero_form_kernel_validation_outputs(&report, &cli.out_dir)?;

    println!("0-form square Matern kernel validation");
    println!(
...
fn print_help() {
    println!(
        "Usage: cargo run --release -p feg-case-studies --example matern_0form_square_kernel_validation -- [options]\n\
\n\
Options:\n\
  --out-dir <path>                 Output directory (default: out/matern_0form_square_kernel_validation)\n\
  --levels <csv>                   Square cells-per-axis levels (default: 32,64,128,256)\n\
  --probe-anchor-level <n>         Anchor level for fixed probe grid (default: 32)\n\
  --spectral-k <n>                 Main spectral mode count cap (default: 64)\n\
  --spectral-sweep-level <n>       Mesh level for spectral k sweep (default: 64)\n\
