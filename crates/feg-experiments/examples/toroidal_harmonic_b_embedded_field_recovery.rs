...
use feg_case_studies::toroidal_harmonic_b::{
    run_toroidal_harmonic_b_embedded_field_recovery, ToroidalHarmonicBConfig,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = run_toroidal_harmonic_b_embedded_field_recovery(&config)?;

    println!("Toroidal embedded-field staged recovery");
    println!(
    ...
    for stage in &result.stages {
        ...
        println!(
            "{}: hall_rmse={:.3e} flux_rmse={:.3e} beta_mean={} alpha_mean={} posterior_precision_nnz={} posterior_factor_nnz={} fill={:.3}x",
            stage.summary.stage,
