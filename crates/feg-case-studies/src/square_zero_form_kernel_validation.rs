...
const DEFAULT_PROBE_ANCHOR_LEVEL: usize = 32;
...
#[derive(Debug, Clone)]
pub struct SquareZeroFormKernelValidationConfig {
...
impl Default for SquareZeroFormKernelValidationConfig {
    fn default() -> Self {
        Self {
            levels: vec![32, 64, 128, 256],
            alpha: DEFAULT_ALPHA,
...
pub struct SquareZeroFormKernelHyperparameters {
    pub dimension: usize,
    pub alpha: f64,
    pub nu: f64,
    pub sigma2: f64,
...
#[derive(Debug, Clone)]
pub struct SquareZeroFormKernelValidationReport {
    pub config: SquareZeroFormKernelValidationConfig,
    pub hyperparameters: SquareZeroFormKernelHyperparameters,
    pub spectral_available: bool,
    pub levels: Vec<SquareZeroFormKernelLevelReport>,
    pub spectral_sweep: Vec<SquareZeroFormSpectralSweepRow>,
}

#[derive(Debug, Clone)]
pub struct SquareZeroFormKernelLevelReport {
    pub level: usize,
    pub ndofs: usize,
    pub eval_indices: Vec<usize>,
    pub eval_points: Vec<Point2>,
    pub observation_indices: Vec<usize>,
    pub observation_local_indices: Vec<usize>,
    pub diagnostics: SquareZeroFormLevelDiagnostics,
    pub metrics: Vec<SquareZeroFormVarianceMetric>,
    ...
    pub correlation_pairs: Vec<SquareZeroFormCorrelationPair>,
    pub correlation_bins: Vec<SquareZeroFormCorrelationBin>,
}

#[derive(Debug, Clone)]
pub struct SquareZeroFormLevelDiagnostics {
    pub level: usize,
    pub cell_width: f64,
...
#[derive(Debug, Clone)]
pub struct SquareZeroFormVarianceMetric {
    pub level: usize,
    pub method: String,
...
#[derive(Debug, Clone)]
pub struct SquareZeroFormCorrelationPair {
    pub i: usize,
    pub j: usize,
    pub distance: f64,
...
#[derive(Debug, Clone)]
pub struct SquareZeroFormCorrelationBin {
    pub bin_index: usize,
...
#[derive(Debug, Clone)]
pub struct SquareZeroFormSpectralSweepRow {
    pub level: usize,
    pub spectral_k: usize,
    pub ndofs: usize,
    pub eval_count: usize,
...
pub fn default_sensor_points() -> Vec<Point2> {
...
pub fn square_zero_form_kernel_hyperparameters(
    alpha: f64,
    sigma2: f64,
    practical_range: f64,
    noise_variance: f64,
) -> Result<SquareZeroFormKernelHyperparameters, String> {
    ...
    Ok(SquareZeroFormKernelHyperparameters {
        dimension,
        alpha,
        nu,
        sigma2,
...
pub fn compute_square_zero_form_kernel_validation_report(
    config: SquareZeroFormKernelValidationConfig,
) -> Result<SquareZeroFormKernelValidationReport, String> {
    validate_config(&config)?;
    let hyperparameters = square_zero_form_kernel_hyperparameters(
        config.alpha,
        config.sigma2,
    ...
    Ok(SquareZeroFormKernelValidationReport {
        config,
        hyperparameters,
        spectral_available,
        levels,
...
pub fn write_square_zero_form_kernel_validation_outputs(
    report: &SquareZeroFormKernelValidationReport,
...
pub fn interior_vertex_indices(coords: &MeshCoords, margin: f64) -> Result<Vec<usize>, String> {
    if coords.dim() != DEFAULT_DIMENSION {
        return Err(format!(
            "expected {DEFAULT_DIMENSION}D square coordinates, got {}D",
            coords.dim()
        ));
    }
...
pub fn fixed_probe_indices_for_mesh(
    ...
    if level == 0 {
        return Err("square level must be positive".to_string());
    }
    ...
    for (idx, coord) in coords.coord_iter().enumerate() {
        ...
        if vertex_by_key.insert(key, idx).is_some() {
            return Err("square mesh produced duplicate vertex grid coordinates".to_string());
        }
    }
...
pub fn correlation_bins(
    ...
    spectral: Option<&[f64]>,
    bin_count: usize,
) -> Result<Vec<SquareZeroFormCorrelationBin>, String> {
    ...
    for bin in 0..bin_count {
        ...
        bins.push(SquareZeroFormCorrelationBin {
            bin_index: bin,
...
fn validate_config(config: &SquareZeroFormKernelValidationConfig) -> Result<(), String> {
    if config.levels.is_empty() {
        return Err("at least one square level is required".to_string());
    }
    if config.levels.contains(&0) {
        return Err("square levels must be positive".to_string());
    }
    ...
    square_zero_form_kernel_hyperparameters(
        config.alpha,
        config.sigma2,
        config.practical_range,
        config.noise_variance,
...
fn compute_level_report(
    level: usize,
    config: &SquareZeroFormKernelValidationConfig,
    hyperparameters: SquareZeroFormKernelHyperparameters,
    spectral_available: bool,
    probe_points: &[Point2],
) -> Result<SquareZeroFormKernelLevelReport, String> {
    ...
    let diagnostics = SquareZeroFormLevelDiagnostics {
        level,
    ...
    metrics.push(SquareZeroFormVarianceMetric {
        level,
    ...
    Ok(SquareZeroFormKernelLevelReport {
        level,
        ndofs,
        eval_indices,
        eval_points,
...
fn compute_spectral_sweep(
    ...
    hyperparameters: SquareZeroFormKernelHyperparameters,
    probe_points: &[Point2],
) -> Result<Vec<SquareZeroFormSpectralSweepRow>, String> {
    ...
    for &requested_k in &config.spectral_sweep_ks {
        ...
        rows.push(SquareZeroFormSpectralSweepRow {
            level,
            spectral_k,
            ndofs,
            eval_count: eval_indices.len(),
...
fn metric_row(
    ...
    covariance_seconds: f64,
    total_seconds: f64,
) -> Result<SquareZeroFormVarianceMetric, String> {
    let variance_rmse = rmse(values, reference)?;
    Ok(SquareZeroFormVarianceMetric {
        level,
...
fn coord_to_point<C>(coord: C) -> Point2
where
...
{
    point_grid_key(&[coord[0], coord[1]], level)
}

fn point_grid_key(point: &Point2, level: usize) -> Result<[i64; DEFAULT_DIMENSION], String> {
...
fn squared_distance_to_coord<C>(point: &Point2, coord: &C) -> f64
where
    C: Index<usize, Output = f64> + ?Sized,
{
    (0..DEFAULT_DIMENSION)
...
fn euclidean_distance(lhs: &Point2, rhs: &Point2) -> f64 {
    (0..DEFAULT_DIMENSION)
        .map(|axis| {
...
fn write_level_variances_csv(
    level: &SquareZeroFormKernelLevelReport,
    path: &Path,
) -> io::Result<()> {
    let mut writer = BufWriter::new(File::create(path)?);
    writeln!(
        writer,
        "vertex_index,x,y,euclidean_variance,gmrf_variance,gmrf_error,gmrf_calibrated_variance,gmrf_calibrated_error,spectral_variance,spectral_error,is_observation"
    )?;
...
fn write_spectral_sweep_svg(
    rows: &[SquareZeroFormSpectralSweepRow],
    path: &Path,
) -> io::Result<()> {
    let points = rows
        .iter()
...
mod tests {
    ...
    #[test]
    fn hyperparameters_match_expected_square_values() {
        let hp = square_zero_form_kernel_hyperparameters(2.0, 1.0, 0.20, 1e-4).unwrap();

        assert_eq!(hp.dimension, 2);
        assert!((hp.nu - 1.0).abs() <= 1e-12);
        assert!((hp.kappa - (8.0_f64).sqrt() / 0.20).abs() <= 1e-12);
        let expected_tau = (1.0 / ((4.0 * std::f64::consts::PI) * hp.kappa.powi(2))).sqrt();
        assert!((hp.tau - expected_tau).abs() <= 1e-12);
    }
    ...
    fn interior_selector_and_sensors_stay_away_from_boundary() {
        ...
        assert_eq!(sensors.iter().copied().collect::<HashSet<_>>().len(), 9);
    }
